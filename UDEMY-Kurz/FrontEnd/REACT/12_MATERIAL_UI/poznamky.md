...
**MUI Form elementy (inputy)**
   ...
   3) rating
          = element pro hodnocení
          - můžeme si určit rozsah + styl (jako všude :D)

**MUI další elementy-NTK**

   1) Typography
          = textové pole
          - pro nadpisy, komentáře, obyčejné texty, ...
          - viz.: nav_menu.jsx

**Stylování MUI (sx prop)**
     = vlastní typ CSS pro stylizaci MUI komponent
     - skupinu spol. komponent můžeme uzavřít do komp. Box = wrapper komponenta čistě pro stylizaci
     - syntaxe př.: <Box sx={{}}> <- styly definujeme do objektu
 
   sx prop
     -viz.: sx_styling.jsx
     - nelze definovat jednotky pro čís. hodnoty
     - hodnoty stylů definujeme jako text pokud jich je víc - "" / '' 
     - některé stylizace fungují jinak
          - p: HODNOTA, -> násobený padding (p: 2, => 2x padding)
          - m: HODNOTA, -> stejné
          -tip - klas. padding a margin funguje :D